...
async function setup() {
  const grp = await db.query("SELECT id FROM tenant_groups WHERE slug='alfalah'");
  if (!grp.rows.length) { console.error('Alfalah group not found — run migrate_alfalah.sql first'); process.exit(1); }
  ...
  const hash = await bcrypt.hash('Alfalah@Pilot2026', 10);
  ...
  `, [tenantId, groupId, 'admin@bankalfalah.com', hash]);

  console.log('Created user: admin@bankalfalah.com / Alfalah@Pilot2026');
