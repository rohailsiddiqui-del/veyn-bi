...
import SignalDrilldownPage from './pages/SignalDrilldownPage';
import { Spinner } from './components/ui';
import { CalendarDays, X } from 'lucide-react';

const PAGE_META = {
  overview:  { title: 'Overview',            sub: 'Performance summary at a glance' },
  quality:   { title: 'Quality Report',      sub: 'Parameter group health, coaching priorities & evidence' },
  ...
  'signal-drilldown': { title: 'Signal Drilldown', sub: 'Keyword search across all flagged & non-flagged calls' },
...
export default function AppShell() {
  ...
  useEffect(() => {
    ...
    else if (user?.dashboard_mode === 'group') setActivePage('overview');
  }, [user?.dashboard_mode]);
  ...
  const isGroup = user?.dashboard_mode === 'group';

  const renderPage = () => {
    // Group mode (Domino's style multi-portal)
    if (isGroup) {
      switch (activePage) {
        case 'overview':  return <GroupOverviewPage />;
        case 'agents':    return <AgentsPage />;
        case 'params':    return <ParamsPage />;
        case 'trend':     return <TrendPage />;
        case 'insights':          return <InsightsPage />;
        case 'signal-drilldown': return <SignalDrilldownPage />;
        case 'upload':            return <UploadPage />;
        case 'settings':          return <AlertSettingsPage />;
        case 'quality':           return <QualityReportPage />;
        default:                  return <GroupOverviewPage />;
      }
    }
    switch (activePage) {
